...
{   
    ...
    PUT(heap_listp + (1*WSIZE), PACK(DSIZE*2, 1)); // 프롤로그 블록의 헤더
    PUT(heap_listp + (2*WSIZE), NULL); // 프롤로그 내 prev
    PUT(heap_listp + (3*WSIZE), NULL); // 프롤로그 내 next
    PUT(heap_listp + (4*WSIZE), PACK(DSIZE*2, 1)); // 프롤로그 블록의 풋터
...
{   
    if (NEXT_FREEB(heap_listp) != NULL){
        PREV_FREEB(NEXT_FREEB(heap_listp)) = bp;
    }

    NEXT_FREEB(bp) = NEXT_FREEB(heap_listp);
    NEXT_FREEB(heap_listp) = bp;
    PREV_FREEB(bp) = heap_listp;
