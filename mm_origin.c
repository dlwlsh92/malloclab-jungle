...
#define DDSIZE 16 // Ddouble word size (bytes)
...
#define PREV_FREEB_PUT(bp, prev) (*((void **)(bp)) = prev) // 이전 가용 블록의 bp
#define NEXT_FREEB_PUT(bp, next) (*((void **)(bp + WSIZE)) = next) // 다음 가용 블록의 bp

#define PRVP(bp) ((char *)(bp)) // prev 포인터
#define NXTP(bp) ((char *)(bp) + WSIZE) //next 포인터
...
int mm_init(void)
{
    if ((heap_listp = mem_sbrk(6*WSIZE)) == (void *)-1) // 프롤로그 및 에필로그 블록 생성을 위한 힙 영역 확장
        return -1;
    PUT(heap_listp, 0); // 힙이 첫번째 블록에 0 삽입
    PUT(heap_listp + (1*WSIZE), PACK(DDSIZE, 1)); // 프롤로그 블록의 헤더
    PUT(heap_listp + (2*WSIZE), NULL); // 프롤로그 내 prev
    PUT(heap_listp + (3*WSIZE), NULL); // 프롤로그 내 next
    PUT(heap_listp + (4*WSIZE), PACK(DDSIZE, 1)); // 프롤로그 블록의 풋터
    PUT(heap_listp + (5*WSIZE), PACK(0, 1)); // 에필로그 블록의 헤더
...
void mm_free(void *bp)
{
    ...
    // printf("coalesce 후 크기: %d\n", GET(HDRP(bp)));
    // printf("free coalesce 종료\n");

}

void insert_link(void *bp)
{   
    // printf("insert_link 시작\n");
    // void *old_block = GET(NXTP(heap_listp)); // root와 연결되어 있던 블록의 bp
    // printf("%p\n", old_block);
    // PUT(NXTP(bp), old_block); // 삽입할 블록의 next를 old_block의 bp로 갱신
    // PUT(NXTP(heap_listp), bp); // root의 next를 삽입할 블록의 bp로 갱신 
    // PUT(PRVP(bp), heap_listp); //삽입할 블록의 prev를 root의 bp로 갱신
    // PUT(PRVP(old_block), bp); // old_block의 prev를 삽입할 블록의 bp로 갱신
    ...
    // #define PUT(p, val) (*(unsigned int *)(p) = (val))
    // printf("insert_link 시작\n");
    PUT(NEXT_FREEB(bp), NEXT_FREEB(heap_listp));
    PUT(PREV_FREEB(bp), heap_listp);
    PUT(NEXT_FREEB(heap_listp), bp);
    
    if (NEXT_FREEB(heap_listp) != NULL){
        PUT(PREV_FREEB(NEXT_FREEB(bp)), bp);
    }

    // printf("insert_link 종료\n");
...
{
    // printf("delete 시작\n");
    // void *prev_block = GET(PRVP(bp)); // 삭제할 블록의 이전 블록 bp
    // void *next_block = GET(NXTP(bp)); // 삭제할 블록의 다음 블록 bp
    
    // PUT(NXTP(prev_block), next_block); // 이전 블록의 next를 다음 블록의 bp로 갱신
    // PUT(PRVP(next_block), prev_block); // 다음 블록의 prev를 이전 블록의 bp로 갱신

    // #define PREV_FREEB(bp) (*(void **)(bp)) // 이전 가용 블록의 bp
    // #define NEXT_FREEB(bp) (*(void **)(bp + WSIZE)) // 다음 가용 블록의 bp

    // #define PRVP(bp) ((char *)(bp)) // prev 포인터 bp
    // #define NXTP(bp) ((char *)(bp) + WSIZE) //next 포인터 bp

    if (NEXT_FREEB(bp) == NULL){
        PUT(NEXT_FREEB(PREV_FREEB(bp)), NEXT_FREEB(bp));
        // printf("현재 블록의 next가 NULL인 경우\n");
        return;
    }

    PUT(NXTP(PREV_FREEB(bp)), NEXT_FREEB(bp));
    PUT(NEXT_FREEB(bp), PREV_FREEB(bp));
...
static void *coalesce(void *bp){
    ...
    printf("coalesce insert_link 시작\n");
    insert_link(bp);
    printf("coalesce 종료\n");
...
static void *find_fit(size_t asize){
    ...
    for (bp = NEXT_FREEB(heap_listp); bp != NULL; bp = NEXT_FREEB(bp)){
        // printf(" find for 문\n");
        // printf("%p\n", NEXT_FREEB(heap_listp));
        // printf("%p\n", bp);
        // printf("%d\n", GET(HDRP(bp)));
        if (!GET_ALLOC(HDRP(bp)) && (asize <= GET_SIZE(HDRP(bp)))){
            // printf("find 탐색 당첨, 사이즈:%d\n", GET(HDRP(bp)));
            // printf(" find 탐색 종료\n");
